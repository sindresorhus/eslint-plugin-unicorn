# Snapshot report for `test/no-instanceof-builtins.js`

The actual snapshot is saved in `no-instanceof-builtins.js.snap`.

Generated by [AVA](https://avajs.dev).

## invalid(1): foo instanceof String

> Input

    `␊
      1 | foo instanceof String␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof String␊
        | ^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'string'\`.:␊
      1 | typeof foo === 'string'␊
    `

## invalid(2): foo instanceof Number

> Input

    `␊
      1 | foo instanceof Number␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof Number␊
        | ^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'number'\`.:␊
      1 | typeof foo === 'number'␊
    `

## invalid(3): foo instanceof Boolean

> Input

    `␊
      1 | foo instanceof Boolean␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof Boolean␊
        | ^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'boolean'\`.:␊
      1 | typeof foo === 'boolean'␊
    `

## invalid(4): foo instanceof BigInt

> Input

    `␊
      1 | foo instanceof BigInt␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof BigInt␊
        | ^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'bigint'\`.:␊
      1 | typeof foo === 'bigint'␊
    `

## invalid(5): foo instanceof Symbol

> Input

    `␊
      1 | foo instanceof Symbol␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof Symbol␊
        | ^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'symbol'\`.:␊
      1 | typeof foo === 'symbol'␊
    `

## invalid(6): foo instanceof Function

> Input

    `␊
      1 | foo instanceof Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof foo === 'function'␊
    `

## invalid(7): foo instanceof Array

> Input

    `␊
      1 | foo instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(foo)␊
    `

## invalid(1): a + b instanceof Function

> Input

    `␊
      1 | a + b instanceof Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a + b instanceof Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof (a + b) === 'function'␊
    `

## invalid(2): a + b instanceof String

> Input

    `␊
      1 | a + b instanceof String␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a + b instanceof String␊
        | ^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'string'\`.:␊
      1 | typeof (a + b) === 'string'␊
    `

## invalid(3): a - b instanceof Function

> Input

    `␊
      1 | a - b instanceof Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a - b instanceof Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof (a - b) === 'function'␊
    `

## invalid(4): (a + b) instanceof Function

> Input

    `␊
      1 | (a + b) instanceof Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (a + b) instanceof Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof (a + b) === 'function'␊
    `

## invalid(1): fooStrict instanceof String

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof String␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof String␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'string'\`.:␊
      1 | typeof fooStrict === 'string'␊
    `

## invalid(2): fooStrict instanceof Number

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Number␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Number␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'number'\`.:␊
      1 | typeof fooStrict === 'number'␊
    `

## invalid(3): fooStrict instanceof Boolean

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Boolean␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Boolean␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'boolean'\`.:␊
      1 | typeof fooStrict === 'boolean'␊
    `

## invalid(4): fooStrict instanceof BigInt

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof BigInt␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof BigInt␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'bigint'\`.:␊
      1 | typeof fooStrict === 'bigint'␊
    `

## invalid(5): fooStrict instanceof Symbol

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Symbol␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Symbol␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'symbol'\`.:␊
      1 | typeof fooStrict === 'symbol'␊
    `

## invalid(6): fooStrict instanceof Function

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof fooStrict === 'function'␊
    `

## invalid(7): fooStrict instanceof Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(fooStrict)␊
    `

## invalid(8): fooStrict instanceof Error

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Error␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Error␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(9): fooStrict instanceof EvalError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof EvalError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof EvalError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(10): fooStrict instanceof RangeError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof RangeError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof RangeError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(11): fooStrict instanceof ReferenceError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof ReferenceError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof ReferenceError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(12): fooStrict instanceof SyntaxError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof SyntaxError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof SyntaxError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(13): fooStrict instanceof TypeError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof TypeError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof TypeError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(14): fooStrict instanceof URIError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof URIError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof URIError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(15): fooStrict instanceof AggregateError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof AggregateError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof AggregateError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(16): fooStrict instanceof SuppressedError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof SuppressedError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof SuppressedError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(17): fooStrict instanceof Map

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Map␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Map␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(18): fooStrict instanceof Set

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Set␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Set␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(19): fooStrict instanceof WeakMap

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof WeakMap␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof WeakMap␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(20): fooStrict instanceof WeakRef

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof WeakRef␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof WeakRef␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(21): fooStrict instanceof WeakSet

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof WeakSet␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof WeakSet␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(22): fooStrict instanceof ArrayBuffer

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof ArrayBuffer␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof ArrayBuffer␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(23): fooStrict instanceof Int8Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Int8Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Int8Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(24): fooStrict instanceof Uint8Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Uint8Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Uint8Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(25): fooStrict instanceof Uint8ClampedArray

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Uint8ClampedArray␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Uint8ClampedArray␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(26): fooStrict instanceof Int16Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Int16Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Int16Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(27): fooStrict instanceof Uint16Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Uint16Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Uint16Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(28): fooStrict instanceof Int32Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Int32Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Int32Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(29): fooStrict instanceof Uint32Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Uint32Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Uint32Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(30): fooStrict instanceof Float16Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Float16Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Float16Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(31): fooStrict instanceof Float32Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Float32Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Float32Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(32): fooStrict instanceof Float64Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Float64Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Float64Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(33): fooStrict instanceof BigInt64Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof BigInt64Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof BigInt64Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(34): fooStrict instanceof BigUint64Array

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof BigUint64Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof BigUint64Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(35): fooStrict instanceof Object

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Object␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Object␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(36): fooStrict instanceof RegExp

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof RegExp␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof RegExp␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(37): fooStrict instanceof Promise

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Promise␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Promise␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(38): fooStrict instanceof Proxy

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Proxy␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Proxy␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(39): fooStrict instanceof DataView

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof DataView␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof DataView␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(40): fooStrict instanceof Date

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof Date␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof Date␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(41): fooStrict instanceof SharedArrayBuffer

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof SharedArrayBuffer␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof SharedArrayBuffer␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(42): fooStrict instanceof FinalizationRegistry

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | fooStrict instanceof FinalizationRegistry␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooStrict instanceof FinalizationRegistry␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(1): fooErr instanceof Error

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'loose'␊
    ␊
    Code:␊
      1 | fooErr instanceof Error␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooErr instanceof Error␊
        | ^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Error.isError(fooErr)␊
    `

## invalid(2): (fooErr) instanceof (Error)

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'loose'␊
    ␊
    Code:␊
      1 | (fooErr) instanceof (Error)␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (fooErr) instanceof (Error)␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Error.isError((fooErr))␊
    `

## invalid(1): err instanceof Error

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof Error␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof Error␊
        | ^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Error.isError(err)␊
    `

## invalid(2): err instanceof EvalError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof EvalError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof EvalError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(3): err instanceof RangeError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof RangeError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof RangeError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(4): err instanceof ReferenceError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof ReferenceError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof ReferenceError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(5): err instanceof SyntaxError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof SyntaxError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof SyntaxError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(6): err instanceof TypeError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof TypeError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof TypeError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(7): err instanceof URIError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof URIError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof URIError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(8): err instanceof AggregateError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof AggregateError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof AggregateError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(9): err instanceof SuppressedError

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
      strategy: 'strict'␊
    ␊
    Code:␊
      1 | err instanceof SuppressedError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | err instanceof SuppressedError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(1): fooInclude instanceof WebWorker

> Input

    `␊
    Options:␊
    - include:␊
        - 'WebWorker'␊
    ␊
    Code:␊
      1 | fooInclude instanceof WebWorker␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooInclude instanceof WebWorker␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(2): fooInclude instanceof HTMLElement

> Input

    `␊
    Options:␊
    - include:␊
        - 'HTMLElement'␊
    ␊
    Code:␊
      1 | fooInclude instanceof HTMLElement␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fooInclude instanceof HTMLElement␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(1): foo instanceof globalThis.String

> Input

    `␊
      1 | foo instanceof globalThis.String␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.String␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Suggestion 1/1: Switch to \`typeof … === 'string'\`.:␊
      1 | typeof foo === 'string'␊
    `

## invalid(2): foo instanceof globalThis /* keep */ .String

> Input

    `␊
      1 | foo instanceof globalThis /* keep */ .String␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis /* keep */ .String␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(3): foo instanceof globalThis.Function

> Input

    `␊
      1 | foo instanceof globalThis.Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | typeof foo === 'function'␊
    `

## invalid(4): foo instanceof globalThis /* keep */ .Function

> Input

    `␊
      1 | foo instanceof globalThis /* keep */ .Function␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis /* keep */ .Function␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(5): foo instanceof globalThis.Array

> Input

    `␊
      1 | foo instanceof globalThis.Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | globalThis.Array.isArray(foo)␊
    `

## invalid(6): const Array = {isArray: () => false}; foo instanceof globalThis.Array

> Input

    `␊
      1 | const Array = {isArray: () => false}; foo instanceof globalThis.Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | const Array = {isArray: () => false}; foo instanceof globalThis.Array␊
        |                                       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | const Array = {isArray: () => false}; globalThis.Array.isArray(foo)␊
    `

## invalid(7): foo instanceof window.Array

> Input

    `␊
      1 | foo instanceof window.Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof window.Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | window.Array.isArray(foo)␊
    `

## invalid(8): foo instanceof self.Array

> Input

    `␊
      1 | foo instanceof self.Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof self.Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | self.Array.isArray(foo)␊
    `

## invalid(9): foo instanceof global.Array

> Input

    `␊
      1 | foo instanceof global.Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof global.Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | global.Array.isArray(foo)␊
    `

## invalid(10): foo instanceof globalThis.Error

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
    ␊
    Code:␊
      1 | foo instanceof globalThis.Error␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.Error␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | globalThis.Error.isError(foo)␊
    `

## invalid(11): const Error = CustomError; foo instanceof globalThis.Error

> Input

    `␊
    Options:␊
    - useErrorIsError: true␊
    ␊
    Code:␊
      1 | const Error = CustomError; foo instanceof globalThis.Error␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | const Error = CustomError; foo instanceof globalThis.Error␊
        |                            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | const Error = CustomError; globalThis.Error.isError(foo)␊
    `

## invalid(12): foo instanceof globalThis.TypeError

> Input

    `␊
    Options:␊
    - strategy: 'strict'␊
    ␊
    Code:␊
      1 | foo instanceof globalThis.TypeError␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.TypeError␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(13): foo instanceof globalThis.WebWorker

> Input

    `␊
    Options:␊
    - include:␊
        - 'WebWorker'␊
    ␊
    Code:␊
      1 | foo instanceof globalThis.WebWorker␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo instanceof globalThis.WebWorker␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    `

## invalid(1): arr instanceof Array

> Input

    `␊
      1 | arr instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | arr instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(arr)␊
    `

## invalid(2): [] instanceof Array

> Input

    `␊
      1 | [] instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | [] instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray([])␊
    `

## invalid(3): [1,2,3] instanceof Array === true

> Input

    `␊
      1 | [1,2,3] instanceof Array === true␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | [1,2,3] instanceof Array === true␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray([1,2,3]) === true␊
    `

## invalid(4): fun.call(1, 2, 3) instanceof Array

> Input

    `␊
      1 | fun.call(1, 2, 3) instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | fun.call(1, 2, 3) instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(fun.call(1, 2, 3))␊
    `

## invalid(5): obj.arr instanceof Array

> Input

    `␊
      1 | obj.arr instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | obj.arr instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(obj.arr)␊
    `

## invalid(6): foo.bar[2] instanceof Array

> Input

    `␊
      1 | foo.bar[2] instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo.bar[2] instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray(foo.bar[2])␊
    `

## invalid(7): (0, array) instanceof Array

> Input

    `␊
      1 | (0, array) instanceof Array␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (0, array) instanceof Array␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | Array.isArray((0, array))␊
    `

## invalid(8): function foo(){return[]instanceof Array}

> Input

    `␊
      1 | function foo(){return[]instanceof Array}␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | function foo(){return[]instanceof Array}␊
        |                      ^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | function foo(){return Array.isArray([])}␊
    `

## invalid(9): ( // comment (( // comment ( // comment foo // comment ) // comment )) // comment ) // comment before instanceof instanceof // comment after instanceof ( // comment ( // comment Array // comment ) // comment ) // comment

> Input

    `␊
       1 | (␊
       2 | 	// comment␊
       3 | 	((␊
       4 | 		// comment␊
       5 | 		(␊
       6 | 			// comment␊
       7 | 			foo␊
       8 | 			// comment␊
       9 | 		)␊
      10 | 		// comment␊
      11 | 	))␊
      12 | 	// comment␊
      13 | )␊
      14 | // comment before instanceof␊
      15 |    instanceof␊
      16 |␊
      17 | // comment after instanceof␊
      18 |␊
      19 | (␊
      20 | 	// comment␊
      21 |␊
      22 | 	(␊
      23 |␊
      24 | 		// comment␊
      25 |␊
      26 | 		Array␊
      27 |␊
      28 | 		// comment␊
      29 | 	)␊
      30 |␊
      31 | 		// comment␊
      32 | )␊
      33 |␊
      34 | 	// comment␊
    `

> Error 1/1

    `␊
    Message:␊
    >  1 | (␊
         | ^␊
    >  2 | 	// comment␊
         | ^^^^^^^^^^^␊
    >  3 | 	((␊
         | ^^^␊
    >  4 | 		// comment␊
         | ^^^^^^^^^^^^␊
    >  5 | 		(␊
         | ^^^␊
    >  6 | 			// comment␊
         | ^^^^^^^^^^^^^␊
    >  7 | 			foo␊
         | ^^^^^^␊
    >  8 | 			// comment␊
         | ^^^^^^^^^^^^^␊
    >  9 | 		)␊
         | ^^^␊
    > 10 | 		// comment␊
         | ^^^^^^^^^^^^␊
    > 11 | 	))␊
         | ^^^␊
    > 12 | 	// comment␊
         | ^^^^^^^^^^^␊
    > 13 | )␊
         | ^␊
    > 14 | // comment before instanceof␊
         | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^␊
    > 15 |    instanceof␊
         | ^^^^^^^^^^^^^␊
    > 16 |␊
         | ^␊
    > 17 | // comment after instanceof␊
         | ^^^^^^^^^^^^^^^^^^^^^^^^^^^␊
    > 18 |␊
         | ^␊
    > 19 | (␊
         | ^␊
    > 20 | 	// comment␊
         | ^^^^^^^^^^^␊
    > 21 |␊
         | ^␊
    > 22 | 	(␊
         | ^^␊
    > 23 |␊
         | ^␊
    > 24 | 		// comment␊
         | ^^^^^^^^^^^^␊
    > 25 |␊
         | ^␊
    > 26 | 		Array␊
         | ^^^^^^^␊
    > 27 |␊
         | ^␊
    > 28 | 		// comment␊
         | ^^^^^^^^^^^^␊
    > 29 | 	)␊
         | ^^␊
    > 30 |␊
         | ^␊
    > 31 | 		// comment␊
         | ^^^^^^^^^^^^␊
    > 32 | )␊
         | ^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
      33 |␊
      34 | 	// comment␊
    ␊
    Output:␊
       1 | Array.isArray((␊
       2 | 	// comment␊
       3 | 	((␊
       4 | 		// comment␊
       5 | 		(␊
       6 | 			// comment␊
       7 | 			foo␊
       8 | 			// comment␊
       9 | 		)␊
      10 | 		// comment␊
      11 | 	))␊
      12 | 	// comment␊
      13 | ))␊
      14 | // comment before instanceof␊
      15 |␊
      16 | // comment after instanceof␊
      17 |␊
      18 | 	// comment␊
      19 |␊
      20 |␊
      21 | 		// comment␊
      22 |␊
      23 |␊
      24 | 		// comment␊
      25 |␊
      26 |␊
      27 | 		// comment␊
      28 |␊
      29 |␊
      30 | 	// comment␊
    `

## invalid(10): <template><div v-if="array instanceof Array" v-for="element of array"></div></template>

> Input

    `␊
      1 | <template><div v-if="array instanceof Array" v-for="element of array"></div></template>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <template><div v-if="array instanceof Array" v-for="element of array"></div></template>␊
        |                      ^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <template><div v-if="Array.isArray(array)" v-for="element of array"></div></template>␊
    `

## invalid(11): <template><div v-if="(( (( array )) instanceof (( Array )) ))" v-for="element of array"></div></template>

> Input

    `␊
      1 | <template><div v-if="(( (( array )) instanceof (( Array )) ))" v-for="element of array"></div></template>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <template><div v-if="(( (( array )) instanceof (( Array )) ))" v-for="element of array"></div></template>␊
        |                         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <template><div v-if="(( Array.isArray((( array ))) ))" v-for="element of array"></div></template>␊
    `

## invalid(12): <template><div>{{(( (( array )) instanceof (( Array )) )) ? array.join(" | ") : array}}</div></template>

> Input

    `␊
      1 | <template><div>{{(( (( array )) instanceof (( Array )) )) ? array.join(" | ") : array}}</div></template>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <template><div>{{(( (( array )) instanceof (( Array )) )) ? array.join(" | ") : array}}</div></template>␊
        |                     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <template><div>{{(( Array.isArray((( array ))) )) ? array.join(" | ") : array}}</div></template>␊
    `

## invalid(13): <script>const foo = array instanceof Array</script>

> Input

    `␊
      1 | <script>const foo = array instanceof Array</script>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <script>const foo = array instanceof Array</script>␊
        |                     ^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <script>const foo = Array.isArray(array)</script>␊
    `

## invalid(14): <script>const foo = (( (( array )) instanceof (( Array )) ))</script>

> Input

    `␊
      1 | <script>const foo = (( (( array )) instanceof (( Array )) ))</script>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <script>const foo = (( (( array )) instanceof (( Array )) ))</script>␊
        |                        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <script>const foo = (( Array.isArray((( array ))) ))</script>␊
    `

## invalid(15): <script>foo instanceof Function</script>

> Input

    `␊
      1 | <script>foo instanceof Function</script>␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | <script>foo instanceof Function</script>␊
        |         ^^^^^^^^^^^^^^^^^^^^^^^ Avoid using \`instanceof\` for type checking as it can lead to unreliable results.␊
    ␊
    Output:␊
      1 | <script>typeof foo === 'function'</script>␊
    `
