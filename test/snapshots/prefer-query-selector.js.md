# Snapshot report for `test/prefer-query-selector.js`

The actual snapshot is saved in `prefer-query-selector.js.snap`.

Generated by [AVA](https://avajs.dev).

## invalid(1): document.getElementById("foo");

> Input

    `␊
      1 | document.getElementById("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById("foo");␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    ␊
    Output:␊
      1 | document.querySelector("#foo");␊
    `

## invalid(2): document.getElementsByClassName("foo");

> Input

    `␊
      1 | document.getElementsByClassName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName("foo");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(".foo");␊
    `

## invalid(3): document.getElementsByClassName("foo bar");

> Input

    `␊
      1 | document.getElementsByClassName("foo bar");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName("foo bar");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(".foo.bar");␊
    `

## invalid(4): document.getElementsByTagName("foo");

> Input

    `␊
      1 | document.getElementsByTagName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName("foo");␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll("foo");␊
    `

## invalid(5): document.getElementById("");

> Input

    `␊
      1 | document.getElementById("");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById("");␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    `

## invalid(6): document.getElementById('foo');

> Input

    `␊
      1 | document.getElementById('foo');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById('foo');␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    ␊
    Output:␊
      1 | document.querySelector('#foo');␊
    `

## invalid(7): document.getElementsByClassName('foo');

> Input

    `␊
      1 | document.getElementsByClassName('foo');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName('foo');␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll('.foo');␊
    `

## invalid(8): document.getElementsByClassName('foo bar');

> Input

    `␊
      1 | document.getElementsByClassName('foo bar');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName('foo bar');␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll('.foo.bar');␊
    `

## invalid(9): document.getElementsByTagName('foo');

> Input

    `␊
      1 | document.getElementsByTagName('foo');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName('foo');␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll('foo');␊
    `

## invalid(10): document.getElementsByClassName('');

> Input

    `␊
      1 | document.getElementsByClassName('');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName('');␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(11): document.getElementById(`foo`);

> Input

    `␊
      1 | document.getElementById(\`foo\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById(\`foo\`);␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    ␊
    Output:␊
      1 | document.querySelector(\`#foo\`);␊
    `

## invalid(12): document.getElementsByClassName(`foo`);

> Input

    `␊
      1 | document.getElementsByClassName(\`foo\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`foo\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(\`.foo\`);␊
    `

## invalid(13): document.getElementsByClassName(`foo bar`);

> Input

    `␊
      1 | document.getElementsByClassName(\`foo bar\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`foo bar\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(\`.foo.bar\`);␊
    `

## invalid(14): document.getElementsByTagName(`foo`);

> Input

    `␊
      1 | document.getElementsByTagName(\`foo\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName(\`foo\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(\`foo\`);␊
    `

## invalid(15): document.getElementsByTagName(``);

> Input

    `␊
      1 | document.getElementsByTagName(\`\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName(\`\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    `

## invalid(16): document.getElementsByClassName(`${fn()}`);

> Input

    `␊
      1 | document.getElementsByClassName(\`${fn()}\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`${fn()}\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(17): document.getElementsByClassName(`foo ${undefined}`);

> Input

    `␊
      1 | document.getElementsByClassName(\`foo ${undefined}\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`foo ${undefined}\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(18): document.getElementsByClassName(null);

> Input

    `␊
      1 | document.getElementsByClassName(null);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(null);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(null);␊
    `

## invalid(19): document.getElementsByTagName(null);

> Input

    `␊
      1 | document.getElementsByTagName(null);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName(null);␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(null);␊
    `

## invalid(20): document.getElementsByClassName(fn());

> Input

    `␊
      1 | document.getElementsByClassName(fn());␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(fn());␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(21): document.getElementsByClassName("foo" + fn());

> Input

    `␊
      1 | document.getElementsByClassName("foo" + fn());␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName("foo" + fn());␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(22): document.getElementsByClassName(foo + "bar");

> Input

    `␊
      1 | document.getElementsByClassName(foo + "bar");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(foo + "bar");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(23): for (const div of document.body.getElementById("id").getElementsByClassName("class")) { console.log(div.getElementsByTagName("div")); }

> Input

    `␊
      1 | for (const div of document.body.getElementById("id").getElementsByClassName("class")) {␊
      2 | 	console.log(div.getElementsByTagName("div"));␊
      3 | }␊
    `

> Error 1/3

    `␊
    Message:␊
    > 1 | for (const div of document.body.getElementById("id").getElementsByClassName("class")) {␊
        |                                 ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
      2 | 	console.log(div.getElementsByTagName("div"));␊
      3 | }␊
    ␊
    Output:␊
      1 | for (const div of document.body.querySelector("#id").getElementsByClassName("class")) {␊
      2 | 	console.log(div.getElementsByTagName("div"));␊
      3 | }␊
    `

> Error 2/3

    `␊
    Message:␊
    > 1 | for (const div of document.body.getElementById("id").getElementsByClassName("class")) {␊
        |                                                      ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
      2 | 	console.log(div.getElementsByTagName("div"));␊
      3 | }␊
    ␊
    Output:␊
      1 | for (const div of document.body.getElementById("id").querySelectorAll(".class")) {␊
      2 | 	console.log(div.getElementsByTagName("div"));␊
      3 | }␊
    `

> Error 3/3

    `␊
    Message:␊
      1 | for (const div of document.body.getElementById("id").getElementsByClassName("class")) {␊
    > 2 | 	console.log(div.getElementsByTagName("div"));␊
        | 	                ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
      3 | }␊
    ␊
    Output:␊
      1 | for (const div of document.body.getElementById("id").getElementsByClassName("class")) {␊
      2 | 	console.log(div.querySelectorAll("div"));␊
      3 | }␊
    `

## invalid(24): e.getElementById(3)

> Input

    `␊
      1 | e.getElementById(3)␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | e.getElementById(3)␊
        |   ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    `

## invalid(25): document.getElementsByName("foo");

> Input

    `␊
      1 | document.getElementsByName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName("foo");␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll("[name='foo']");␊
    `

## invalid(26): document.getElementsByName('foo');

> Input

    `␊
      1 | document.getElementsByName('foo');␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName('foo');␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll('[name="foo"]');␊
    `

## invalid(27): document.getElementsByName(`foo`);

> Input

    `␊
      1 | document.getElementsByName(\`foo\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName(\`foo\`);␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(\`[name='foo']\`);␊
    `

## invalid(28): document.getElementsByName(`${'foo'}`);

> Input

    `␊
      1 | document.getElementsByName(\`${'foo'}\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName(\`${'foo'}\`);␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    `

## invalid(29): document.getElementsByName(null);

> Input

    `␊
      1 | document.getElementsByName(null);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName(null);␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(null);␊
    `

## invalid(30): document.getElementsByName("");

> Input

    `␊
      1 | document.getElementsByName("");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName("");␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    `

## invalid(31): document.getElementsByName(foo + "bar");

> Input

    `␊
      1 | document.getElementsByName(foo + "bar");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName(foo + "bar");␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    `

## invalid(32): document.getElementsByName("multiple name should be fixable");

> Input

    `␊
      1 | document.getElementsByName("multiple name should be fixable");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName("multiple name should be fixable");␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll("[name='multiple name should be fixable']");␊
    `

## invalid(33): document.getElementById("foo");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementById("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById("foo");␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    ␊
    Output:␊
      1 | document.querySelector("#foo");␊
    `

## invalid(34): document.getElementsByClassName("foo");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName("foo");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(".foo");␊
    `

## invalid(35): document.getElementsByTagName("foo");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByTagName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName("foo");␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll("foo");␊
    `

## invalid(36): document.getElementsByName("foo");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByName("foo");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName("foo");␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll("[name='foo']");␊
    `

## invalid(37): document.getElementsByTagName(someTag);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByTagName(someTag);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByTagName(someTag);␊
        |          ^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByTagName()\`.␊
    `

## invalid(38): document.getElementsByName(someName);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByName(someName);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByName(someName);␊
        |          ^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByName()\`.␊
    `

## invalid(39): document.getElementsByClassName(null);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName(null);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(null);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(null);␊
    `

## invalid(40): document.getElementsByClassName(`foo`);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName(\`foo\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`foo\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    ␊
    Output:␊
      1 | document.querySelectorAll(\`.foo\`);␊
    `

## invalid(41): document.getElementsByClassName(variable + "x");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName(variable + "x");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(variable + "x");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(42): document.getElementsByClassName("foo" + fn());

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName("foo" + fn());␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName("foo" + fn());␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(43): document.getElementsByClassName(foo + "bar");

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName(foo + "bar");␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(foo + "bar");␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `

## invalid(44): document.getElementById(`x${someId}`);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementById(\`x${someId}\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementById(\`x${someId}\`);␊
        |          ^^^^^^^^^^^^^^ Prefer \`.querySelector()\` over \`.getElementById()\`.␊
    `

## invalid(45): document.getElementsByClassName(`foo ${someClass}`);

> Input

    `␊
    Options:␊
    - allowWithVariables: true␊
    ␊
    Code:␊
      1 | document.getElementsByClassName(\`foo ${someClass}\`);␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | document.getElementsByClassName(\`foo ${someClass}\`);␊
        |          ^^^^^^^^^^^^^^^^^^^^^^ Prefer \`.querySelectorAll()\` over \`.getElementsByClassName()\`.␊
    `
