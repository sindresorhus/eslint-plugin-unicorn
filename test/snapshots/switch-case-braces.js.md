# Snapshot report for `test/switch-case-braces.js`

The actual snapshot is saved in `switch-case-braces.js.snap`.

Generated by [AVA](https://avajs.dev).

## invalid(1): switch(foo){ case 1: { } case 2: { } default: { doSomething(); } }

> Input

    `␊
      1 | switch(foo){␊
      2 | 	case 1: {␊
      3 | 	}␊
      4 | 	case 2: {␊
      5 | 	}␊
      6 | 	default: {␊
      7 | 		doSomething();␊
      8 | 	}␊
      9 | }␊
    `

> Error 1/2

    `␊
    Message:␊
      1 | switch(foo){␊
    > 2 | 	case 1: {␊
        | 	        ^ Unexpected braces in empty case clause.␊
      3 | 	}␊
      4 | 	case 2: {␊
      5 | 	}␊
      6 | 	default: {␊
      7 | 		doSomething();␊
      8 | 	}␊
      9 | }␊
    ␊
    Output:␊
      1 | switch(foo){␊
      2 | 	case 1:␊
      3 | 	case 2: {␊
      4 | 	}␊
      5 | 	default: {␊
      6 | 		doSomething();␊
      7 | 	}␊
      8 | }␊
    `

> Error 2/2

    `␊
    Message:␊
      1 | switch(foo){␊
      2 | 	case 1: {␊
      3 | 	}␊
    > 4 | 	case 2: {␊
        | 	        ^ Unexpected braces in empty case clause.␊
      5 | 	}␊
      6 | 	default: {␊
      7 | 		doSomething();␊
      8 | 	}␊
      9 | }␊
    ␊
    Output:␊
      1 | switch(foo){␊
      2 | 	case 1: {␊
      3 | 	}␊
      4 | 	case 2:␊
      5 | 	default: {␊
      6 | 		doSomething();␊
      7 | 	}␊
      8 | }␊
    `

## invalid(2): switch(foo){ case 1: { // fallthrough } default: { } // fallthrough case 3: { doSomething(); break; } }

> Input

    `␊
       1 | switch(foo){␊
       2 | 	case 1: {␊
       3 | 		// fallthrough␊
       4 | 	}␊
       5 | 	default: {␊
       6 | 	}␊
       7 | 	// fallthrough␊
       8 | 	case 3: {␊
       9 | 		doSomething();␊
      10 | 		break;␊
      11 | 	}␊
      12 | }␊
    `

> Error 1/2

    `␊
    Message:␊
       1 | switch(foo){␊
    >  2 | 	case 1: {␊
         | 	        ^ Unexpected braces in empty case clause.␊
       3 | 		// fallthrough␊
       4 | 	}␊
       5 | 	default: {␊
       6 | 	}␊
       7 | 	// fallthrough␊
       8 | 	case 3: {␊
       9 | 		doSomething();␊
      10 | 		break;␊
      11 | 	}␊
      12 | }␊
    ␊
    Output:␊
       1 | switch(foo){␊
       2 | 	case 1:␊
       3 | 		// fallthrough␊
       4 | 	default: {␊
       5 | 	}␊
       6 | 	// fallthrough␊
       7 | 	case 3: {␊
       8 | 		doSomething();␊
       9 | 		break;␊
      10 | 	}␊
      11 | }␊
    `

> Error 2/2

    `␊
    Message:␊
       1 | switch(foo){␊
       2 | 	case 1: {␊
       3 | 		// fallthrough␊
       4 | 	}␊
    >  5 | 	default: {␊
         | 	         ^ Unexpected braces in empty case clause.␊
       6 | 	}␊
       7 | 	// fallthrough␊
       8 | 	case 3: {␊
       9 | 		doSomething();␊
      10 | 		break;␊
      11 | 	}␊
      12 | }␊
    ␊
    Output:␊
       1 | switch(foo){␊
       2 | 	case 1: {␊
       3 | 		// fallthrough␊
       4 | 	}␊
       5 | 	default:␊
       6 | 	// fallthrough␊
       7 | 	case 3: {␊
       8 | 		doSomething();␊
       9 | 		break;␊
      10 | 	}␊
      11 | }␊
    `

## invalid(1): switch(foo) { default: doSomething(); }

> Input

    `␊
      1 | switch(foo) {␊
      2 | 	default:␊
      3 | 		doSomething();␊
      4 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default:␊
        | 	^^^^^^^^ Missing braces in case clause.␊
      3 | 		doSomething();␊
      4 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default: {␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | }␊
    `

## invalid(2): switch(foo) { case 1: { doSomething(); } break; // <-- This should be between braces; }

> Input

    `␊
      1 | switch(foo) {␊
      2 | 	case 1: {␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | 	break; // <-- This should be between braces;␊
      6 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	case 1: {␊
        | 	^^^^^^^ Missing braces in case clause.␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | 	break; // <-- This should be between braces;␊
      6 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	case 1: { {␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | 	break; // <-- This should be between braces;␊
      6 | 	}␊
      7 | }␊
    `

## invalid(3): switch(foo) { default: label: {} }

> Input

    `␊
      1 | switch(foo) {␊
      2 | 	default:␊
      3 | 		label: {}␊
      4 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default:␊
        | 	^^^^^^^^ Missing braces in case clause.␊
      3 | 		label: {}␊
      4 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default: {␊
      3 | 		label: {}␊
      4 | 	}␊
      5 | }␊
    `

## invalid(1): switch(foo) { default: { doSomething(); } }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	default: {␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default: {␊
        | 	         ^ Unnecessary braces in case clause.␊
      3 | 		doSomething();␊
      4 | 	}␊
      5 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default:␊
      3 | 		doSomething();␊
      4 | }␊
    `

## invalid(2): switch(foo) { default: { /* block comment */ doSomething(); } }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	default: {␊
      3 | 		/* block comment */ doSomething();␊
      4 | 	}␊
      5 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default: {␊
        | 	         ^ Unnecessary braces in case clause.␊
      3 | 		/* block comment */ doSomething();␊
      4 | 	}␊
      5 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default:␊
      3 | 		/* block comment */ doSomething();␊
      4 | }␊
    `

## invalid(3): switch(foo) { default: { { const bar = 2; doSomething(); } doSomethingElse(); } }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	default: {␊
      3 | 		{␊
      4 | 			const bar = 2;␊
      5 | 			doSomething();␊
      6 | 		}␊
      7 | 		doSomethingElse();␊
      8 | 	}␊
      9 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default: {␊
        | 	         ^ Unnecessary braces in case clause.␊
      3 | 		{␊
      4 | 			const bar = 2;␊
      5 | 			doSomething();␊
      6 | 		}␊
      7 | 		doSomethingElse();␊
      8 | 	}␊
      9 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default:␊
      3 | 		{␊
      4 | 			const bar = 2;␊
      5 | 			doSomething();␊
      6 | 		}␊
      7 | 		doSomethingElse();␊
      8 | }␊
    `

## invalid(4): switch(foo) { case 1: { break; } }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	case 1: {␊
      3 | 		break;␊
      4 | 	}␊
      5 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	case 1: {␊
        | 	        ^ Unnecessary braces in case clause.␊
      3 | 		break;␊
      4 | 	}␊
      5 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	case 1:␊
      3 | 		break;␊
      4 | }␊
    `

## invalid(5): switch(foo) { default: {{{ doSomething(); }}} }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	default: {{{␊
      3 | 		doSomething();␊
      4 | 	}}}␊
      5 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default: {{{␊
        | 	         ^ Unnecessary braces in case clause.␊
      3 | 		doSomething();␊
      4 | 	}}}␊
      5 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default:{{␊
      3 | 		doSomething();␊
      4 | 	}}␊
      5 | }␊
    `

## invalid(6): switch(foo) { default: {{{ doSomething(); { doSomethingElseInBlockStatement(); } }}} }

> Input

    `␊
    Options:␊
    - 'avoid'␊
    ␊
    Code:␊
      1 | switch(foo) {␊
      2 | 	default: {{{␊
      3 | 		doSomething();␊
      4 | 		{␊
      5 | 			doSomethingElseInBlockStatement();␊
      6 | 		}␊
      7 | 	}}}␊
      8 | }␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | switch(foo) {␊
    > 2 | 	default: {{{␊
        | 	         ^ Unnecessary braces in case clause.␊
      3 | 		doSomething();␊
      4 | 		{␊
      5 | 			doSomethingElseInBlockStatement();␊
      6 | 		}␊
      7 | 	}}}␊
      8 | }␊
    ␊
    Output:␊
      1 | switch(foo) {␊
      2 | 	default:{{␊
      3 | 		doSomething();␊
      4 | 		{␊
      5 | 			doSomethingElseInBlockStatement();␊
      6 | 		}␊
      7 | 	}}␊
      8 | }␊
    `
