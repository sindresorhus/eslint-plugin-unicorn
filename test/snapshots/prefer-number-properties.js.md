...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | (Number.parseFloat)("10.5");␊
    `

## invalid(47): parseFloat(foo as string);

> Input

    `␊
      1 | parseFloat(foo as string);␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | Number.parseFloat(foo as string);␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | Number.isNaN(foo >>> 0);␊
    `

## invalid(6): isNaN(foo.length);
...
## invalid(7): isNaN(parseFloat(foo));
...
> Error 2/2
    ...
    ␊
    Output:␊
      1 | isNaN(Number.parseFloat(foo));␊
    `

## invalid(8): isNaN(Number(foo));
...
## invalid(9): isFinite(Math.floor(foo));
...
## invalid(10): const foo = 5; isNaN(foo);
...
## invalid(11): function foo(bar: number) { return isNaN(bar); }
...
## invalid(12): isNaN(foo as number);
...
## invalid(13): isNaN(foo satisfies number);
...
## invalid(15): function foo(bar: number) { return isNaN(bar!); }
...
## invalid(16): const foo: number = bar(); isFinite(foo);
...
## invalid(25): isNaN(foo as Number);
...
## invalid(26): const foo: Number = bar(); isFinite(foo);
