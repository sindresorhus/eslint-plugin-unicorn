...
## invalid(18): foo > (0,bar) ? foo : (0,bar)

> Input

    `␊
      1 | foo > (0,bar) ? foo : (0,bar)␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | Math.max(foo, (0,bar))␊
    `

## invalid(19): Number(x) > 50 ? 50 : Number(x)

> Input

    `␊
      1 | Number(x) > 50 ? 50 : Number(x)␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | Math.min(Number(x), 50)␊
    `

## invalid(20): foo.bar() > 10 ? 10 : foo.bar()
...
## invalid(21): async function foo() { return await foo.bar() > 10 ? 10 : await foo.bar() }
...
## invalid(22): async function foo() { await(+foo > 10 ? 10 : +foo) }
...
## invalid(23): function foo() { return(foo.bar() > 10) ? 10 : foo.bar() }
...
## invalid(24): function* foo() { yield+foo > 10 ? 10 : +foo }
...
## invalid(25): export default+foo > 10 ? 10 : +foo
...
## invalid(26): foo.length > bar.length ? bar.length : foo.length
...
## invalid(7): function foo(a, b) { return (a as number) > b ? (a as number) : b; }

> Input

    `␊
      1 | function foo(a, b) {␊
      2 | 	return (a as number) > b ? (a as number) : b;␊
      3 | }␊
    `
...
## invalid(8): function foo(a, b) { return (<number>a) > b ? (<number>a) : b; }

> Input

    `␊
      1 | function foo(a, b) {␊
      2 | 	return (<number>a) > b ? (<number>a) : b;␊
      3 | }␊
    `
...
> Error 1/1
    ...
    Output:␊
      1 | function foo(a, b) {␊
      2 | 	return Math.max(<number>a, b);␊
      3 | }␊
    `

## invalid(9): function foo(a, b) { return a! > b ? a! : b; }
