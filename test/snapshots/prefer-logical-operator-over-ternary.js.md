# Snapshot report for `test/prefer-logical-operator-over-ternary.js`

The actual snapshot is saved in `prefer-logical-operator-over-ternary.js.snap`.

Generated by [AVA](https://avajs.dev).

## invalid(1): foo ? foo : bar;

> Input

    `␊
      1 | foo ? foo : bar;␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo ? foo : bar;␊
        | ^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo ?? bar;␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo || bar;␊
    `

## invalid(2): foo.bar ? foo.bar : foo.baz

> Input

    `␊
      1 | foo.bar ? foo.bar : foo.baz␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo.bar ? foo.bar : foo.baz␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo.bar ?? foo.baz␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo.bar || foo.baz␊
    `

## invalid(3): foo?.bar ? foo.bar : baz

> Input

    `␊
      1 | foo?.bar ? foo.bar : baz␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo?.bar ? foo.bar : baz␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo?.bar ?? baz␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo?.bar || baz␊
    `

## invalid(4): !bar ? foo : bar;

> Input

    `␊
      1 | !bar ? foo : bar;␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | !bar ? foo : bar;␊
        | ^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | bar ?? foo;␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | bar || foo;␊
    `

## invalid(5): !!bar ? foo : !bar;

> Input

    `␊
      1 | !!bar ? foo : !bar;␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | !!bar ? foo : !bar;␊
        | ^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | !bar ?? foo;␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | !bar || foo;␊
    `

## invalid(6): foo() ? foo() : bar

> Input

    `␊
      1 | foo() ? foo() : bar␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo() ? foo() : bar␊
        | ^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo() ?? bar␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo() || bar␊
    `

## invalid(7): foo ? foo : a && b

> Input

    `␊
      1 | foo ? foo : a && b␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo ? foo : a && b␊
        | ^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo ?? (a && b)␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo || (a && b)␊
    `

## invalid(8): foo ? foo : a || b

> Input

    `␊
      1 | foo ? foo : a || b␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo ? foo : a || b␊
        | ^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo ?? (a || b)␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo || a || b␊
    `

## invalid(9): foo ? foo : a ?? b

> Input

    `␊
      1 | foo ? foo : a ?? b␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo ? foo : a ?? b␊
        | ^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo ?? a ?? b␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo || (a ?? b)␊
    `

## invalid(10): a && b ? a && b : bar

> Input

    `␊
      1 | a && b ? a && b : bar␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a && b ? a && b : bar␊
        | ^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (a && b) ?? bar␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (a && b) || bar␊
    `

## invalid(11): a || b ? a || b : bar

> Input

    `␊
      1 | a || b ? a || b : bar␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a || b ? a || b : bar␊
        | ^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (a || b) ?? bar␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | a || b || bar␊
    `

## invalid(12): a ?? b ? a ?? b : bar

> Input

    `␊
      1 | a ?? b ? a ?? b : bar␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | a ?? b ? a ?? b : bar␊
        | ^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | a ?? b ?? bar␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (a ?? b) || bar␊
    `

## invalid(13): foo ? foo : await a

> Input

    `␊
      1 | foo ? foo : await a␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo ? foo : await a␊
        | ^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo ?? (await a)␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo || (await a)␊
    `

## invalid(14): await a ? await a : foo

> Input

    `␊
      1 | await a ? await a : foo␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | await a ? await a : foo␊
        | ^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (await a) ?? foo␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (await a) || foo␊
    `

## invalid(15): const foo = [] !+a ? b : +a

> Input

    `␊
      1 | const foo = []␊
      2 | !+a ? b : +a␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | const foo = []␊
    > 2 | !+a ? b : +a␊
        | ^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | const foo = []␊
      2 | ;+a ?? b␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | const foo = []␊
      2 | ;+a || b␊
    `

## invalid(16): const foo = [] a && b ? a && b : 1

> Input

    `␊
      1 | const foo = []␊
      2 | a && b ? a && b : 1␊
    `

> Error 1/1

    `␊
    Message:␊
      1 | const foo = []␊
    > 2 | a && b ? a && b : 1␊
        | ^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | const foo = []␊
      2 | ;(a && b) ?? 1␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | const foo = []␊
      2 | ;(a && b) || 1␊
    `

## invalid(17): (foo as string) ? (foo as string) : bar;

> Input

    `␊
      1 | (foo as string) ? (foo as string) : bar;␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (foo as string) ? (foo as string) : bar;␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (foo as string) ?? bar;␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (foo as string) || bar;␊
    `

## invalid(18): (foo.bar as string) ? (foo.bar as string) : foo.baz

> Input

    `␊
      1 | (foo.bar as string) ? (foo.bar as string) : foo.baz␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (foo.bar as string) ? (foo.bar as string) : foo.baz␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (foo.bar as string) ?? foo.baz␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (foo.bar as string) || foo.baz␊
    `

## invalid(19): (a && b as boolean) ? (a && b as boolean) : bar

> Input

    `␊
      1 | (a && b as boolean) ? (a && b as boolean) : bar␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | (a && b as boolean) ? (a && b as boolean) : bar␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | (a && b as boolean) ?? bar␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | (a && b as boolean) || bar␊
    `

## invalid(20): foo! ? foo! : bar;

> Input

    `␊
      1 | foo! ? foo! : bar;␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo! ? foo! : bar;␊
        | ^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo! ?? bar;␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo! || bar;␊
    `

## invalid(21): foo.bar! ? foo.bar! : foo.baz

> Input

    `␊
      1 | foo.bar! ? foo.bar! : foo.baz␊
    `

> Error 1/1

    `␊
    Message:␊
    > 1 | foo.bar! ? foo.bar! : foo.baz␊
        | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Prefer using a logical operator over a ternary.␊
    ␊
    Suggestion 1/2: Switch to \`??\` operator.:␊
      1 | foo.bar! ?? foo.baz␊
    ␊
    Suggestion 2/2: Switch to \`||\` operator.:␊
      1 | foo.bar! || foo.baz␊
    `
