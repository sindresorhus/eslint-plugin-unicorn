...
## invalid(7): for (let i = 0, len = sideEffect().length; i < arr.length; i++) { console.log(arr[i]); }

> Input

    `␊
      1 | for (let i = 0, len = sideEffect().length; i < arr.length; i++) { console.log(arr[i]); }␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | for (const element of arr) { console.log(element); }␊
    `

## invalid(8): for (var i = 0, len = arr.length; i < len; i++) { console.log(arr[i]); } console.log(len);
