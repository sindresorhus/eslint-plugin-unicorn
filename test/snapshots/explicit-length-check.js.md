...
## invalid(6): const foo = {length: -1}; [foo.length] = [123]; if (foo.length) {}
...
## invalid(10): const foo = {length: -1}; for (foo.length in {key: true}) {} if (foo.length) {}

> Input

    `␊
      1 | const foo = {length: -1}; for (foo.length in {key: true}) {} if (foo.length) {}␊
    `
...
> Error 1/1
    ...
    ␊
    Output:␊
      1 | const foo = {length: -1}; for (foo.length in {key: true}) {} if (foo.length > 0) {}␊
    `

## invalid(11): if (foo.bar && foo.bar.length) {}
...
## invalid(12): if (foo.length || foo.bar()) {}
...
## invalid(16): do {} while (foo.length);
...
## invalid(17): for (let i = 0; (bar && !foo.length); i ++) {}
...
## invalid(18): const isEmpty = foo.length < 1;
...
## invalid(19): const isEmpty = foo.length < 1 ? true : false;
...
## invalid(20): const isEmpty = foo.length <= 0;
...
## invalid(23): bar(!foo.length || foo.length)
...
## invalid(24): const bar = void !foo.length;
...
## invalid(25): const isNotEmpty = Boolean(foo.length)
...
## invalid(26): if (!!Boolean(foo.length)) {}
...
## invalid(27): const isNotEmpty = Boolean(foo.length || bar)
...
## invalid(28): const isEmpty = Boolean(!foo.length)
...
## invalid(29): const isEmpty = Boolean(foo.length === 0)
...
## invalid(30): const isNotEmpty = !Boolean(foo.length === 0)
...
## invalid(31): const isEmpty = !Boolean(!Boolean(foo.length === 0))
...
## invalid(33): if (foo.size && bar.length) {}
...
## invalid(34): function foo() {return!foo.length}
...
## invalid(35): function foo() {throw!foo.length}
...
## invalid(36): async function foo() {await!foo.length}
...
## invalid(38): function * foo() {yield*!foo.length}
...
## invalid(42): a instanceof!foo.length
...
## invalid(44): export default!foo.length
...
## invalid(45): if(true){}else!foo.length
...
## invalid(46): do!foo.length;while(true) {}
...
## invalid(47): switch(foo){case!foo.length:{}}
...
## invalid(48): for(const a of!foo.length);
...
## invalid(49): for(const a in!foo.length);
