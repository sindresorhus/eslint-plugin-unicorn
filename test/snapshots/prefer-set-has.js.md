...
## invalid(43): const items = [1, 2, 3]; const foo = items.slice(); foo.includes(1) || foo.includes(2);
...
## invalid(44): const items = [1, 2, 3]; const foo = items.concat(4); foo.includes(1) || foo.includes(2);
...
## invalid(45): const foo = _([1,2,3]); const bar = foo.map(value => value); function unicorn() { return bar.includes(1); }
