...
## invalid(32): const foo = await Promise.race([promise])
...
## invalid(33): const foo = () => Promise.race([promise])
...
## invalid(34): foo = await Promise.race([promise])
...
## invalid(37): await Promise.race([(x,y)]) [0].toString()
...
## invalid(1): const foo = () => Promise.race([promise])
...
## invalid(2): const foo = await Promise.all([promise])
...
## invalid(3): foo = await Promise.all([promise])
...
> Error 1/1
    ...
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Switch to \`Promise.resolve(…)\`.␊
      1 | foo = await Promise.resolve(promise)␊
    `
