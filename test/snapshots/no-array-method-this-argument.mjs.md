# Snapshot report for `test/no-array-method-this-argument.mjs`

The actual snapshot is saved in `no-array-method-this-argument.mjs.snap`.

Generated by [AVA](https://avajs.dev).

## Invalid #1
      1 | array.every(() => {}, thisArgument)

> Output

    `␊
      1 | array.every(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.every(() => {}, thisArgument)␊
        |                       ^^^^^^^^^^^^ Do not use this argument in \`Array#every()\`.␊
    `

## Invalid #2
      1 | array.filter(() => {}, thisArgument)

> Output

    `␊
      1 | array.filter(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.filter(() => {}, thisArgument)␊
        |                        ^^^^^^^^^^^^ Do not use this argument in \`Array#filter()\`.␊
    `

## Invalid #3
      1 | array.find(() => {}, thisArgument)

> Output

    `␊
      1 | array.find(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.find(() => {}, thisArgument)␊
        |                      ^^^^^^^^^^^^ Do not use this argument in \`Array#find()\`.␊
    `

## Invalid #4
      1 | array.findIndex(() => {}, thisArgument)

> Output

    `␊
      1 | array.findIndex(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.findIndex(() => {}, thisArgument)␊
        |                           ^^^^^^^^^^^^ Do not use this argument in \`Array#findIndex()\`.␊
    `

## Invalid #5
      1 | array.flatMap(() => {}, thisArgument)

> Output

    `␊
      1 | array.flatMap(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.flatMap(() => {}, thisArgument)␊
        |                         ^^^^^^^^^^^^ Do not use this argument in \`Array#flatMap()\`.␊
    `

## Invalid #6
      1 | array.forEach(() => {}, thisArgument)

> Output

    `␊
      1 | array.forEach(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.forEach(() => {}, thisArgument)␊
        |                         ^^^^^^^^^^^^ Do not use this argument in \`Array#forEach()\`.␊
    `

## Invalid #7
      1 | array.map(() => {}, thisArgument)

> Output

    `␊
      1 | array.map(() => {})␊
    `

> Error 1/1

    `␊
    > 1 | array.map(() => {}, thisArgument)␊
        |                     ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    `

## Invalid #8
      1 | array.map(() => {}, thisArgument,)

> Output

    `␊
      1 | array.map(() => {},)␊
    `

> Error 1/1

    `␊
    > 1 | array.map(() => {}, thisArgument,)␊
        |                     ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    `

## Invalid #9
      1 | array.map(() => {}, (0, thisArgument),)

> Output

    `␊
      1 | array.map(() => {},)␊
    `

> Error 1/1

    `␊
    > 1 | array.map(() => {}, (0, thisArgument),)␊
        |                      ^^^^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    `

## Invalid #10
      1 | array.map(() => {}, thisArgumentHasSideEffect())

> Error 1/1

    `␊
    > 1 | array.map(() => {}, thisArgumentHasSideEffect())␊
        |                     ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/1: Remove the second argument.␊
      1 | array.map(() => {})␊
    `

## Invalid #1
      1 | array.map(callback, thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(callback, thisArgument)␊
        |                     ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(callback)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(callback.bind(thisArgument))␊
    `

## Invalid #2
      1 | array.map(callback, (0, thisArgument))

> Error 1/1

    `␊
    > 1 | array.map(callback, (0, thisArgument))␊
        |                      ^^^^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(callback)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(callback.bind((0, thisArgument)))␊
    `

## Invalid #3
      1 | array.map(function () {}, thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(function () {}, thisArgument)␊
        |                           ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(function () {})␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(function () {}.bind(thisArgument))␊
    `

## Invalid #4
      1 | array.map(function callback () {}, thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(function callback () {}, thisArgument)␊
        |                                    ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(function callback () {})␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(function callback () {}.bind(thisArgument))␊
    `

## Invalid #5
      1 | array.map(new Callback, thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(new Callback, thisArgument)␊
        |                         ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(new Callback)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map((new Callback).bind(thisArgument))␊
    `

## Invalid #6
      1 | array.map(1, thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(1, thisArgument)␊
        |              ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(1)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map((1).bind(thisArgument))␊
    `

## Invalid #7
      1 | async () => array.map(await callback, thisArgument)

> Error 1/1

    `␊
    > 1 | async () => array.map(await callback, thisArgument)␊
        |                                       ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | async () => array.map(await callback)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | async () => array.map((await callback).bind(thisArgument))␊
    `

## Invalid #8
      1 | array.map((new Callback), thisArgument)

> Error 1/1

    `␊
    > 1 | array.map((new Callback), thisArgument)␊
        |                           ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map((new Callback))␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map((new Callback).bind(thisArgument))␊
    `

## Invalid #9
      1 | array.map(new Callback(), thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(new Callback(), thisArgument)␊
        |                           ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(new Callback())␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(new Callback().bind(thisArgument))␊
    `

## Invalid #10
      1 | array.map( (( callback )), (( thisArgument )),)

> Error 1/1

    `␊
    > 1 | array.map( (( callback )), (( thisArgument )),)␊
        |                               ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map( (( callback )),)␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map( (( callback )).bind((( thisArgument ))),)␊
    `

## Invalid #11
      1 | array.map((0, () => {}), thisArgument)

> Error 1/1

    `␊
    > 1 | array.map((0, () => {}), thisArgument)␊
        |                          ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map((0, () => {}))␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map((0, () => {}).bind(thisArgument))␊
    `

## Invalid #12
      1 | array.map(callback.bind(foo), thisArgument)

> Error 1/1

    `␊
    > 1 | array.map(callback.bind(foo), thisArgument)␊
        |                               ^^^^^^^^^^^^ Do not use this argument in \`Array#map()\`.␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 1/2: Remove the second argument.␊
      1 | array.map(callback.bind(foo))␊
    ␊
    --------------------------------------------------------------------------------␊
    Suggestion 2/2: Use bound function.␊
      1 | array.map(callback.bind(foo).bind(thisArgument))␊
    `
