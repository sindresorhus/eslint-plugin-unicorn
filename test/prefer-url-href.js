...
test.snapshot({
	valid: [
		...
		'String(url); const url = new URL(value);',
		'const url2 = url; const url = new URL(value); url2.toString();',
	...
	invalid: [
		...
		typescript('import type {URL} from "node:url"; new URL(value).toString();'),
		typescript('import type {URL} from "node:url"; String(new URL(value));'),
		...
		typescript('const url: URL | string = new URL(value); String(url);'),
		typescript('const url: unknown = new URL(value); url.toString();'),
...
test.snapshot({
	valid: [
		...
		typeAware('declare const url: import("node:url").URL | string; String(url);'),
