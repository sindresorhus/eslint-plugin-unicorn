...
test.snapshot({
	valid: [
		...
		'JSON.parse(await fs.readFile?.(file));',
		'JSON.parse(await fs?.readFile(file));',
		'JSON.parse(await fs.notReadFileMethod(file));',
		'JSON.parse?.(await fs.readFile(file));',
		'JSON?.parse(await fs.readFile(file));',
		'window.JSON.parse(await fs.readFile(file));',
		'JSON.stringify(await fs.readFile(file));',
		'NOT_JSON.parse(await fs.readFile(file));',
		'for (const string of []) JSON.parse(string);',
		'JSON.parse(await fs.readFile(file), extraArgument);',
		'JSON.parse(foo);',
		'JSON.parse();',
		...
		outdent`
			const foo = {};
			foo.bar = await fs.readFile(file);
			JSON.parse(foo.bar);
		`,
		outdent`
			const foo = await fs.readFile(file);
			const bar = await foo;
			console.log(baz);
			const baz = await bar;
			JSON.parse(baz);
	...
	invalid: [
		...
		`,
		outdent`
			const foo = await await fs.readFile(file);
			const bar = await await foo;
			const baz = await bar;
			JSON.parse(baz);
		`,
		outdent`
			var foo = await fs.readFile(file);
			let bar = await foo;
			const baz = await bar;
			JSON.parse(baz);
		`,
		...
		outdent`
			...
			const foo = buffer;
			async function fn1() {
				const buffer = fs.readFile(file); /* Should NOT report */
...
const buffer = testCase => ({
	...testCase,
	options: ['buffer'],
});

const bufferCode = code => buffer({code});
