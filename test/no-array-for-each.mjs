...
test.typescript({
	...
	invalid: [
		...
		},
		{
			...
			`,
			output: outdent`
				if (staticPages) for (const pg of staticPages)  allStaticPages.add(pg)
				pageInfos?.forEach((info: PageInfo, key: string) => {
					allPageInfos.set(key, info)
				})
			`,
		...
		},
		{
			...
			output: outdent`
				const cloakVals: string[] = [];
				if (elements) for (const element of elements)  cloakVals.push(cloakElement(element));
			`,
			errors: 1,
		},
	],
...
test({
	...
	invalid: [
		...
		},
		{
			...
			output: outdent`
				while (true) return;
				if (foo) for (const element of foo)  bar(element);
			`,
			errors: 1,
			parserOptions: globalReturnOptions,
		},
