...
ruleTest.snapshot({
	valid: [
		...
		'const Boolean = value => value; const re = /a/; const bar = Boolean(foo.match(re))',
		'const Boolean = value => value; const isNotEmpty = Boolean(uri.match(/unicorn/).length)',
		'function unicorn(Boolean) { const re = /a/; if (Boolean(foo.match(re))) {} }',
