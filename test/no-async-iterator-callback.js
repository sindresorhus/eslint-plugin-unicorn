...
test.snapshot({
	...
	invalid: [
		...
		'function * generate() { yield 1; } generate().filter(async value => value);',
...
test.snapshot({
	...
	valid: [
		...
		'declare const iterator: unknown; (iterator as AsyncIterator<number>).filter(async value => value);',
	...
	invalid: [
		...
		'function foo(iterator: IteratorObject<number> | undefined) { iterator!.map(value => value).filter(async value => value); }',
		...
		'declare const iterator: unknown; (iterator as IteratorObject<number>).filter(async value => value);',
		'declare const iterator: unknown; (<IteratorObject<number>>iterator).filter(async value => value);',
...
test.snapshot({
	valid: [
		...
		typeAware('declare const callback: () => boolean | undefined; Iterator.from([1]).filter(callback);'),
		typeAware('function foo<T extends boolean>(callback: () => T) { Iterator.from([1]).filter(callback); }'),
	...
	invalid: [
		...
		typeAware('function foo<T extends boolean | PromiseLike<boolean>>(callback: () => T) { Iterator.from([1]).filter(callback); }'),
