...
const functionWhitelist = new Set(['Boolean']);

const calleeBlacklist = ['Promise', 'React.children', '_', 'Async', 'async'];

const isIteratorMethod = node =>
	node.callee.property && iteratorMethods.has(node.callee.property.name);
const hasFunctionArgument = node =>
	node.arguments.length > 0 &&
	(node.arguments[0].type === 'Identifier' ||
		node.arguments[0].type === 'CallExpression') &&
	!functionWhitelist.has(node.arguments[0].name);

const getNumberOfArguments = node =>
	node.callee.property && iteratorMethods.get(node.callee.property.name);
...
const fix = (context, node) => {
	...
	return fixer =>
		fixer.replaceText(
			argument,
			`${
				numberOfArguments === 1 ? argumentString : `(${argumentString})`
			} => ${parseArgument(context, argument)}(${argumentString})`
		);
};

const toSelector = name => {
	const splitted = name.split('.');
	return `[callee.${'object.'.repeat(
		splitted.length
	)}name!="${splitted.shift()}"]`;
};
	...
	[selector]: node => {
		if (
			...
			context.report({
				node: argument,
				message:
					'Do not pass a function reference directly to an iterator method.',
