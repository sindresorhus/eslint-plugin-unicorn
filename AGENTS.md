...
- `toml/toml`: [`eslint-plugin-toml`](https://github.com/ota-meshi/eslint-plugin-toml)
...
- `TOMLValue` nodes distinguish strings, integers, floats, and dates/times through `kind`. Read numeric source text to preserve precision and signs; skip `inf` and `nan` when grouping digits. Do not convert `1.0` to `1`, which changes the TOML type.
- The parser exposes `#` comments as `Block` nodes. Use `getComments(context)` and preserve `#` when fixing comments. `sourceCode.parserServices?.isTOML` identifies TOML.
- Basic strings and basic quoted keys interpret escapes; literal strings do not. `escape-case` demonstrates escape-preserving edits. `string-content` checks decoded string values only, leaves keys unchanged, and writes double-quoted basic strings. Its serializer uses `JSON.stringify()` plus DEL escaping and skips fixes for unpaired surrogates.
- Use `languages.toml` from `test/utils/test.js` in snapshot cases. For regular rule-tester tests, use `language: languages.toml.language` and `plugins: languages.toml.plugins`. Test fixes and suggestions with the TOML parser, including literal/multiline strings and comments.
...
- **Non-JavaScript language table** in `readme.md` is generated from `meta.languages`. Run `npm run fix:non-js-languages` after changing language support; add new language columns in `scripts/create-non-js-languages-list.js`.
