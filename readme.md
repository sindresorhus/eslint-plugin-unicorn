...
| [no-unscoped-css-nesting-selector](docs/rules/no-unscoped-css-nesting-selector.md)                         | Disallow unscoped CSS nesting selectors.                                                                                       | 🎨✅ 🎨☑️      |    |    |    |
...
The `css/recommended` and `css/unopinionated` configs enable compatible rules for CSS. Extend them from a config that sets `language: 'css/css'` and loads `@eslint/css`, as shown in the [non-JavaScript files example](#non-javascript-files).
