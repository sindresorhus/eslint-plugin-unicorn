...
| [no-unnecessary-splice](docs/rules/no-unnecessary-splice.md)                                     | Bans `array.splice()` calls where it's a no-op or can be replaced with `shift`/`unshift`/`pop`/`push` calls                                                                                                       | ✅  | 🔧 |    |
