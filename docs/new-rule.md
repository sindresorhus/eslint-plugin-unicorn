...
1. Fixes must not remove comments. If a fix replaces or removes a range, use `wouldRemoveComments()` from `rules/utils/` and report without a fix when comments would be lost. If only a smaller range matters, use `hasCommentsInside()`.
