...
In TypeScript, these ternaries are reported without a fix unless full type information proves that the condition's type is exactly `boolean` at that occurrence. Literal types, generic boolean constraints, and other narrowed or asserted types are not automatically fixed because logical operators can change inferred types and overload resolution.
