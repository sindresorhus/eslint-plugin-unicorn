...
Calls to [`String#repeat()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/repeat) on string literals and no-substitution template literals whose values do not contain `$` are also allowed. Repetition cannot introduce a replacement pattern when the original string does not contain `$`.
