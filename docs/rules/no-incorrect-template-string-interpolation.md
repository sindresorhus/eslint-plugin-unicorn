...
This rule is intentionally narrow. It does not report arbitrary expression text like `{name + suffix}` or other placeholder syntaxes like `{{name}}`, and it ignores tagged template literals because tags often use braces for other languages such as HTML, CSS, GraphQL, or i18n templates.
