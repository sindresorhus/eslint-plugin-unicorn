...
Dynamic expression-only templates wrapped in TypeScript `as const` or `<const>` assertions are ignored because `String()` can change their type or invalidate the assertion.
