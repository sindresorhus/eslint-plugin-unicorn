...
The rule does not change custom property names or values, custom functions or their contents, other author-defined names beginning with `--`, font feature value names, type selectors, classes, IDs, attribute or environment variable data, strings, URL payloads, or user data in selector arguments. It also leaves `!important` and encoding strings to their dedicated rules.
