...
The rule does not recognize pipelines split across statements, imperative loops, lookup tables, other padding idioms, arbitrary regex patterns, optional operations, computed method names, or `toString('hex', start, end)` ranges. Direct typed-array `map()` encoders are ignored because typed-array mapping coerces the callback's strings back into numbers.
