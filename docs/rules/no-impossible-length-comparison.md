...
The `length` and `size` properties of built-in collections and strings represent a count, so they cannot be negative. Comparing them against a negative number is either always `false` or always `true`, which usually means the wrong value or comparison operator was used.
